...
def get_operation_name(node):
    ...
    if isinstance(node, dict):
        ...
        for key, inner_node in node.items():
            ...
            if own.has_key('operationName') and isinstance(inner_node, str):
                own[node_operation_name] = inner_node
                #print("   FOUND OPERATION_NAME")
            if own.has_key('duration') and isinstance(inner_node, int):
                own[node_duration] = inner_node
